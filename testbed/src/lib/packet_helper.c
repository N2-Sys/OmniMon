...
enum PACKET_STATUS decode(const uint8_t* pkt,
    ...
    if (eth_len == 14) {
        ...
        else if (ntohs(eth_hdr->ether_type) != ETHER_TYPE_IPv4) {
            status = STATUS_NON_IP;
        }
    }
    else if (eth_len == 4) {
        if (ntohs(*(uint16_t*)(pkt + 2)) != ETHER_TYPE_IPv4) {
            status = STATUS_NON_IP;
        }
    }
    else if (eth_len != 0) {
    ...
    if (ip_hdr->ip_p == IPPROTO_TCP) {
        ...
        p->fin_rst_flag = (uint8_t)tcp_hdr->syn | tcp_hdr->rst;
    }
    else if (ip_hdr->ip_p == IPPROTO_UDP) {
        // UDP
...
/*
#ifdef KIRA
void read_tuple(char* line1, char* line2, tuple_t* p, struct FlowMetric * fm) {
    unsigned int ip1, ip2, ip3, ip4, ip5, ip6, ip7, ip8;
    sscanf(line1, "%u.%u.%u.%u(%hu) <-> %u.%u.%u.%u(%hu)",
        &ip1, &ip2, &ip3, &ip4,
        &p->key.src_port,
        &ip5, &ip6, &ip7, &ip8,
        &p->key.dst_port, 
    ...
    sscanf(line2, "%u %u %u %u %u %d",
        &fm->pkt_cnt, &fm->byte_cnt,
        &fm->index1, &fm->index2,
        &fm->version, &fm->position);
}
#else
*/
