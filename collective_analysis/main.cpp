...
vector<int> host_ids;
vector<int> switch_ids;
int n_host = 0;
int n_switch = 0;

void read_device(char* dir) {
    char filename[100];
    char line[100];
    sprintf(filename, "%s/device_ids.txt", dir);
    FILE* fp = fopen(filename, "r");

    while (fgets(line, 100, fp) != NULL) {
        string s = line;
        int pos = s.find(" ");
        int id = stoi(s.substr(pos+1));
        if (s.substr(0, pos) == "host") {
            host_ids.push_back(id);
        }
        else {
            switch_ids.push_back(id);
        }
    }

    n_host = host_ids.size();
    n_switch = switch_ids.size();
